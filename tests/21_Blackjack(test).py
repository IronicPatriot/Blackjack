...
###### Plan/notes ######
# have dealers first card be hidden (score will show only known cards)
# make version with terminal and without
# aces are working now but keep an eye on them
# change desktop icon (compiling changes to bitmap, running through python gives python icon)
...
'''
# add dealer face down card like a real casino
card_back = 'images/card-back.png'
mystery_value = 0
def dealer_secret_card():
    global mystery_value, dealer_hand
    card_image = ImageTk.PhotoImage(Image.open(card_back).resize((180, 245), Image.ANTIALIAS))
    ...
    for x in dealer_hand: #without the zero it gives 380
        mystery_value += card_value[x.split('-')[0]]

        if "images/ace" in x.split('-'):
            mystery_value = x.split('-').count("images/ace")
            if mystery_value > 2 or print_num > 21:
                print_num += -10
...
def new_game():
    ...
    frames()
    '''
    deck.append(dealer_hand.clear())
    deck.append(player_hand.clear())
    random.shuffle(deck)    # deck append and or shuffle is broken
    '''
    dealer_hand.clear()
    player_hand.clear()
    suits = ['hearts', 'clubs', 'diamonds', 'spades']
...
hit_button.grid(row=2, column=1) # has to be on seperate line otherwise disabling doesn't work
